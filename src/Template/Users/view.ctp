<nav class="large-3 medium-4 columns" id="actions-sidebar">
	<ul class="side-nav">
		...
		<li><?= $this->Form->postLink('Delete',['action'=>'delete',$user->id],['confirm'=>__('Are you sure you want to delete {0}?',$user->display_name)]) ?></li>
	</ul>
</nav>
<div class="users view large-9 medium-8 columns content">
    ...
    <table class="vertical-table">
        ...
        <tr>
            ...
            <td><?= ($this->Number->format($user->position) == 1) ? 'Admin' : 'Member' ?></td>
        </tr>
        <tr>
            <th scope="row"><?= __('Status') ?></th>
            <td><?= ($this->Number->format($user->status) == 1) ? 'Active' : 'Deactivated' ?></td>
        </tr>
        <tr>
